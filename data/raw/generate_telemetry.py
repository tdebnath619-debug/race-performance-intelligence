"""Synthetic Bahrain-inspired 15-corner telemetry generator."""
import numpy as np, pandas as pd
from pathlib import Path
RNG=np.random.default_rng(2024); LAP_M=5412
CORNERS=[(310,80,120,150),(550,115,148,80),(750,75,110,130),(1050,155,190,60),
         (1350,120,158,90),(1580,145,178,70),(1820,95,130,110),(2100,65,100,150),
         (2380,110,145,90),(2650,165,205,50),(3000,85,125,120),(3280,130,168,75),
         (3500,70,108,140),(3780,95,132,110),(4100,155,200,55)]
DRS_ZONES=[(4300,5100),(200,700)]
def _speed_profile(noise=0.4):
    d=np.linspace(0,LAP_M,LAP_M*3); v=np.full_like(d,310.0)
    for entry,apex_v,exit_v,brk in CORNERS:
        b_end=entry+brk; mb=(d>=entry)&(d<b_end)
        if mb.sum(): t=(d[mb]-entry)/brk; v[mb]=np.minimum(v[mb],310*(1-t)+apex_v*t)
        ma=(d>=b_end)&(d<b_end+40); v[ma]=np.minimum(v[ma],apex_v)
        es,ee=b_end+40,min(b_end+200,LAP_M); me=(d>=es)&(d<ee)
        if me.sum(): t=(d[me]-es)/max(ee-es,1); v[me]=np.minimum(v[me],apex_v*(1-t)+exit_v*t)
    return d,np.clip(v,60,340)+RNG.normal(0,noise,len(d))
def _build_lap(dists,speeds,lap_num,fuel_start=90.0):
    ...
    gear=np.where(speeds<80,1,np.where(speeds<120,2,np.where(speeds<160,3,np.where(speeds<200,4,np.where(speeds<240,5,np.where(speeds<280,6,np.where(speeds<315,7,8)))))))
    rpm=np.clip(gear*2200+speeds*18+RNG.normal(0,80,n),4000,18000)
    steering=np.zeros(n)
    for entry,*_ in CORNERS:
        mask=(dists>=entry)&(dists<entry+120)
        if mask.sum(): steering[mask]=RNG.choice([-1,1])*np.clip(np.linspace(0,0.6,mask.sum())+RNG.normal(0,0.02,mask.sum()),-1,1)
    ...
    return pd.DataFrame({"time":ts.round(4),"distance":dists.round(2),"speed":speeds.round(2),
        "throttle":throttle.round(4),"brake":brake.round(4),"gear":gear.astype(int),
        "rpm":rpm.round(0).astype(int),"steering":steering.round(4),"drs_state":drs,
        "ers_deployment_kw":ers_dep.round(2),"ers_harvesting_kw":ers_har.round(2),
...
def generate(out_path="data/raw/telemetry.csv",n_laps=2):
    ...
    for lap in range(n_laps):
        dists,speeds=_speed_profile(0.4 if lap==0 else 0.9)
        df=_build_lap(dists,speeds,lap,fuel)
        df["time"]+=t_off; df["lap_number"]=lap; df["lap_time"]=df["time"]-t_off
        t_off=float(df["time"].iloc[-1])+0.01; fuel=float(df["fuel_kg"].iloc[-1]); frames.append(df)
