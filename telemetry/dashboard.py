...
def generate(analysis_path="reports/analysis.json",
    ...
    findings_html = "".join("<div class='finding'>- "+f+"</div>" for f in findings) or "<div class='finding'>- "+faster+" faster by "+"{:.3f}".format(abs(total_dt))+" s</div>"

    metrics = "".join(
        "<div class='mc'><div class='ml'>"+l+"</div><div class='mv'>"+str(lap.get(k,"-"))+"</div><div class='ms'>"+u+"</div></div>"
        for l,k,u in [
            ("Lap time","lap_time_s","seconds"),("Max speed","max_speed_kmh","km/h"),
            ...
            ("Fuel","fuel_load_kg","kg"),("Top gear","top_gear",""),
        ]
    )
    ...
    CSS = ("*{box-sizing:border-box;margin:0;padding:0}"
           ...
           ".hdr p{font-size:12px;color:#555}"
           ...
           ".tab{padding:11px 16px;font-size:13px;font-weight:500;color:#555;cursor:pointer;border-bottom:2px solid transparent;white-space:nowrap;background:none;border-top:none;border-left:none;border-right:none}"
           ".tab:hover{color:#aaa}"
           ".tab.active{color:#fff;border-bottom-color:#e10600}"
           ".panel{display:none;padding:20px 24px}"
           ".panel.active{display:block}"
           ".mg{display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:8px;margin-bottom:16px}"
           ...
           ".mv{font-size:21px;font-weight:600;color:#fff}"
           ...
           ".finding{background:#111;border-left:3px solid #e10600;padding:8px 12px;border-radius:0 6px 6px 0;font-size:13px;color:#bbb;margin-bottom:6px}"
    ...
    JS = ("(function(){"
          ...
          "var corners=pd.corners||[];"
          "var N=300,L=pd.lap_length_m||5412;"
          "var labels=[],speeds=[];"
          "for(var i=0;i<N;i++){"
          ...
          "new Chart(document.getElementById('sc'),{type:'line',"
          "  data:{labels:labels,datasets:[{label:'Speed (km/h)',data:speeds,"
          "    borderColor:'#e8002d',borderWidth:1.5,pointRadius:0,tension:0.4}]},options:base});"
          "var o2=JSON.parse(JSON.stringify(base));o2.scales.y.min=0;o2.scales.y.max=1;"
          "new Chart(document.getElementById('tc'),{type:'line',"
          "  data:{labels:labels,datasets:["
          "    {label:'Throttle',data:thr,borderColor:'#4ade80',borderWidth:1.5,pointRadius:0,tension:0.3},"
          "    {label:'Brake',data:brk,borderColor:'#f87171',borderWidth:1.5,pointRadius:0,tension:0.3}]},"
          "  options:o2});"
          "})();")
    ...
    html = (
        ...
        "<thead><tr><th>Turn</th><th>Type</th><th>Entry</th><th>Apex</th><th>Exit</th>"
        "<th>Brk dist</th><th>Lat G</th><th>Time</th><th>Score</th></tr></thead>"
        "<tbody>"+crow+"</tbody></table></div></div>"
        "<div id='delta' class='panel'>"
        "<div class='sec'>Key findings</div>"+findings_html+
        "<div class='sec' style='margin-top:16px'>Corner delta &mdash; "+driver_a+" vs "+driver_b+"</div>"
        "<div class='card'><table>"
        "<thead><tr><th>Turn</th><th>Type</th><th>Delta t</th><th>Dv Entry</th>"
        "<th>Dv Apex</th><th>Dv Exit</th><th colspan=3>Narrative</th></tr></thead>"
        "<tbody>"+delta_rows+"</tbody></table></div></div>"
        "<div id='chart' class='panel'>"
        "<div class='sec'>Speed profile</div>"
        "<div class='card'><div class='cw'><canvas id='sc'></canvas></div></div>"
        "<div class='sec'>Throttle and brake</div>"
        "<div class='card'><div class='cw'><canvas id='tc'></canvas></div></div></div>"
