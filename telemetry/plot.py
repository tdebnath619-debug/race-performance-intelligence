"""telemetry/plot.py — Comparison chart."""
import logging
...
def plot_comparison(df_a,df_b,cum_delta,common_dist,label_a="A",label_b="B",out_path=None,show=False):
    if not PLOT: log.warning("matplotlib not available."); return None
    ...
    panels=[("speed","Speed (km/h)"),("throttle","Throttle"),("brake","Brake"),("ers_deployment_kw","ERS Deploy (kW)")]
    axes=[]
    for idx,(col,ylabel) in enumerate(panels):
        ax=fig.add_subplot(gs[idx]); ax.set_facecolor("#0d0d0d")
    ...
    ax_d=fig.add_subplot(gs[3]); ax_d.set_facecolor("#0d0d0d")
